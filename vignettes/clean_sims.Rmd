...
run_simulation <- function(n1,n,mean,sd,B=10,resam=100,perms=100,...){
    ...
    results %>% bind_rows %>% summarize(ASN=mean(ne),maxSN=max(ne),permtest=mean(permtest),permtest2=mean(permtest2),invnorm=mean(invnorm),npcomb=mean(npcomb))
}
...
```{r invnorm}
run_simulation_scont <- function(n1,n,mean,sd,cmean,csd,cprop,B=10,resam=100,perms=100,...){
    results <- mclapply2(1:B,function(i) compare_adaptive_tests(n1,n,cond_power_rule_norm,rnorm_scont,diffmean,mean=mean,sd=sd,cmean=cmean,csd=csd,cprop=cprop,resam=resam,perms=perms,...))
    results %>% bind_rows %>% summarize(ASN=mean(ne),maxSN=max(ne),permtest=mean(permtest),permtest2=mean(permtest2),invnorm=mean(invnorm),npcomb=mean(npcomb))
}
...
sim <- run_simulation_scont(6,12,1,1,3,.1,.1,B=1000,perms=10000,resam=10000)
