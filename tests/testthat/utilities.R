...
largeX <- matrix(rnorm(2*n*5000),nc=5000)
g <- rep(c(0,1),each=n)
...
test_that("Test of zstat function",
          {
              ...
              expect_aboutequal(sd(zstat(largeX,g)),1)
              expect_aboutequal(mean(zstat(largeX,g)),0)
          })
...
test_that("Test pairwise stuff",
          {
              ...
              expect_aboutequal(pw_meandiff(x,g2)*n*2/3,flip(x,pw_contrasts(g2),statTest='sum')@res$Stat)
              expect_aboutequal(as.numeric(t(pw_meandiff(X,g2)*n*2/3)),flip(X,pw_contrasts(g2),statTest='sum')@res$Stat)
          })
